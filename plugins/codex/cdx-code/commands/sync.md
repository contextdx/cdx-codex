...
- `sent: true` → print `📊 Coverage snapshot sent — <percent>% analysed, <pending> pending, <staleNodes> stale node(s).` If `pending > 0` or `staleNodes > 0`, add: `Close the gap with /analyze (incremental).`
