...
Work the intent queue for this board. An **intent** is an architect-authored work item: a proposed change to the codebase, anchored to board elements, that you implement (or reject) from this session. Resolutions flow back to the architect's board.
...
- `intent.payload` — `kind: "directive"` (instruction only) or `kind: "board_diff"` with `suggestions[]` (GraphSuggestion rows: add/remove/modify nodes or edges the change should make true in the codebase)
- `anchors[]` — each board element the intent is about, resolved against the local board JSON:
  ...
  - `resolved: false` with a `note` — e.g. no local board data (suggest running `/analyze`) or the element wasn't matched; locate the code by the element's slug/name instead
...
1. `implemented` is recorded now, but it is only **proven** when a later `/sync` push matches the proposed change — the server stamps `verifiedAt` at that point. Suggest: run `/analyze` then `/sync` after committing so the architect sees the intent verified.
